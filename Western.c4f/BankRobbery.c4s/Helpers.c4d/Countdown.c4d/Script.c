...
{
    SetPosition(64,98);        // Positionieren 66,124
...
func FxCountingTimer(pTarget, iNumber)
{
  ...
  for(pCounter in FindObjects(Find_ID(GetID())) )
  {
    ...
    // Und anzeigen
    CustomMessage(szMessage, pCounter, GetOwner(pCounter), +1,+45, iColor);
  }
...
func FxCounterBlinkTimer(pTarget, iNumber)
{
  ...
  while(pCounter = FindObject(GetID(), 0, 0, 0, 0, 0, 0, 0, 0, pCounter) )
  {
    ...
    // Und anzeigen
    CustomMessage(szMessage, pCounter, GetOwner(pCounter), +1,+45, 0xff0000);
    // Blinken
