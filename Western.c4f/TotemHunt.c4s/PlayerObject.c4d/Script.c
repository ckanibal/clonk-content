...
//   if(Contained(GetCrew(iPlr, i))) continue;
 ...
 iRelaunchTimer = GetRelaunchTimer(iPlr);
 SetAction("Relaunch");
 SetOwner(iPlr);
...
local iRelaunchTimer;

public func GetRelaunchTimer(int iPlayer)
{
  var iTimer;
  if(iTimer = GameCall("GetRelaunchTimer", iPlayer)) return(iTimer);
  return(30);
}
...
private func Execute()
{
  ...
  if(!iRelaunchTimer) return(DoPlrLaunch());
  PlrMessage("$MsgRelaunchTime$",iPlr,iRelaunchTimer/60,(iRelaunchTimer/10)%6,iRelaunchTimer%10);
  iRelaunchTimer--;
}
