...
private func CheckHit()
{
  ...
  for(var i = iSteps; i; i--)
  {
    ...
    if(pObj = FindObject2(Sort_Distance(),Find_AtPoint(iX,iY),Find_NoContainer(),
			  Find_Layer(GetObjectLayer()),Find_Exclude(this()),
			  Find_Or(Find_OCF(OCF_Alive),Find_Func("IsBulletTarget",this()))))
      return(HitLiving(pObj));
