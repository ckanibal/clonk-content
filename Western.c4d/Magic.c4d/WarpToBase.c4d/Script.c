...
private func Warp(clonk)
{
  // Basis?
  var home;
  if(!(home = FindMainTent(GetClan(), 0) )) return(0, Message("$TxtNobase$",clonk) );
...
public func GetSpellCombo(pMage) { return ("16"); } // (1: Backward; 2: Down; 3: Forward; 4: Throw; 5: Jump; 6: Dig)
