...
public func PalisadeTop()      { return(-15); }
public func PalisadeRange()    { return(20);  } // Maximale Ausrichtungsentfernung
public func PalisadeXSub()     { return(0);   } // Das Palisadenstück steht nicht über, das Türme brauchen das
...
public func AdjustPalisadePosition()
{
  ...
  if (pPart = FindPalisade( -iWdtH - iAdjustRange))
    if (InPalisadeRange( GetX() - (i = GetX(pPart) + pPart->~GetObjWidth()/2 + iWdtH - pPart->~PalisadeXSub() - PalisadeXSub()) ))
      SetPosition(i, GetY());
