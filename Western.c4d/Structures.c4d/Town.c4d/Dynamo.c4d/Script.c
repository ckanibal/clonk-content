...
func StartWork(pWorker)
  {
  if(GetID(pWorker) != COWB) return(Message("$TxtCowboysOnly$", pWorker, GetName(0, GetID(pWorker))));
...
func Working()
  ...
  if(!FindContents(COWB)) return(SetAction("Idle"));
  if(!GetAlive(FindContents(COWB))) return(WorkCancel(FindContents(COWB)));
  if(GetCommand(FindContents(COWB))) return(WorkCancel(FindContents(COWB)));
  CheckForSolid();
  return(DoEnergy(+2));
  }
...
private func WorkCancel(pWorker)
  {
  if(!pWorker) pWorker=FindContents(COWB);
  // Arbeit abbrechen
